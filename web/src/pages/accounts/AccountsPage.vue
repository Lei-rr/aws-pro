...
import { clearAccountsCache } from '@/features/accounts'
...
const filtered = computed(() => {
  ...
  return accounts.value.filter((a) =>
    [a.id, a.access_key, a.remark].some((v) =>
      String(v || '')
        .toLowerCase()
        .includes(kw)
    )
      ...
      <Table :key="tableKey">
        ...
        <TableBody class="**:data-[slot=table-cell]:py-2.5">
          ...
          <TableRow v-for="record in pagedAccounts" :key="record.id">
            ...
            <TableCell class="max-w-[14rem] truncate" :title="record.access_key">{{
              record.access_key || '—'
            }}</TableCell>
            ...
            <TableCell class="whitespace-nowrap">
              ...
              <Button
                variant="link"
                size="sm"
                class="text-destructive h-auto px-2"
                :disabled="!!deletingId"
                @click="remove(record)"
              >
