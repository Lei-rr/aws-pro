...
import { newbieApi } from '@/features/newbie'
import { apiPayload } from '@/shared/api/http'
import { toast } from '@/shared/lib/toast'
import { errorMessage } from '@/shared/lib/errors'
import { confirmDialog } from '@/shared/ui/confirm'

import type { NewbieTask } from '@/shared/api/types'
...
function openStream(taskId: string, version: number) {
  ...
  stream.onmessage = (event) => {
    if (version !== watchVersion || eventSource !== stream) return
...
function humanizeAwsCredentialError(raw: unknown): string {
  ...
  if (
    /could not load credentials|Missing credentials|credentials/i.test(text) &&
    /invalid|missing|not found/i.test(text)
  ) {
...
async function startTask(ownerAccountId: string, ownerStep: string) {
  ...
  loading.value = true
  terminalNotified = false
  const ownerStepLabel = stepOptions.find((item) => item.value === ownerStep)?.label || ownerStep
  logs.value = [`正在创建后台任务（账号: ${ownerAccountId}，范围: ${ownerStepLabel}）...`]
  stopWatching()
  lastLogSeq = 0
  task.value = null
  try {
    ...
    if (ownerAccountId === accountId.value && ownerStep === step.value) loading.value = false
  }
}
...
async function confirmStart() {
  ...
  const ownerAccountId = accountId.value
  const ownerStep = step.value
  const isAll = ownerStep === 'all'
  if (
    !(await confirmDialog({
      title: '确认执行新手任务',
      description: isAll
  ...
  <div class="flex flex-1 flex-col gap-4">
    ...
    <div class="space-y-3">
      <div>
        ...
        <p class="text-muted-foreground text-sm">选择账号与执行范围后开始；运行中可终止，日志可单独清空。</p>
      </div>
      <FieldGroup class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        ...
        <Field class="sm:col-span-2 lg:col-span-1">
          ...
          <div class="flex flex-wrap items-center gap-2">
            ...
            <Button size="sm" variant="outline" :disabled="running" @click="clearLog"> 清空日志 </Button>
          </div>
        </Field>
      </FieldGroup>
    </div>
