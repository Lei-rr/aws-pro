...
import { regionsApi } from '@/features/regions'
...
async function enableRegion(row: RegionRow) {
  const owner = accountId.value
  if (!owner) return
  const busyKey = `${owner}::${row.region}`
  if (enabling.value.has(busyKey)) return
  if (
    !(await confirmDialog({
      title: '启用区域',
  ...
  enabling.value = new Set(enabling.value).add(busyKey)
  try {
    await regionsApi.enable({ account_id: owner, region: row.region })
    if (owner !== accountId.value) return
    row.status = 'ENABLING'
    toast.success('已提交启用请求')
  } catch (e) {
    if (owner === accountId.value) toast.error(errorMessage(e, '启用区域失败'))
  } finally {
          ...
          <TableRow v-for="record in pagedItems" :key="`${record.account_id}:${record.region}`">
            ...
            <TableCell>
              <Button
                ...
                size="sm"
                class="h-auto px-0"
                :disabled="enabling.has(`${accountId}::${record.region}`)"
