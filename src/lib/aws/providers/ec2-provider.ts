...
export class Ec2Provider {
  ...
  private instanceView(account: AwsAccount, region: string, instance: any, staticIps: Record<string, string>): Ec2Instance | null {
  ...
  private async resolveAmi(client: EC2Client, key: string) {
    ...
    const images = [...(result.Images ?? [])].sort((a, b) => providerString(b.CreationDate).localeCompare(providerString(a.CreationDate)))
  ...
  private hasVpcIpv6Association(vpc: any) {
    return (vpc.Ipv6CidrBlockAssociationSet ?? []).some((a: any) => ['associating', 'associated'].includes(providerString(a.Ipv6CidrBlockState?.State)))
  }
  private compareSubnet = (a: any, b: any) => providerString(a.AvailabilityZone).localeCompare(providerString(b.AvailabilityZone)) || providerString(a.SubnetId).localeCompare(providerString(b.SubnetId))
