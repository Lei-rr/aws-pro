...
import type { AwsAccount, Ec2Instance } from '../../shared/aws/aws.types.js'
...
export class Ec2Provider {
      ...
      do {
        ...
        for (const reservation of result.Reservations ?? []) {
          for (const instance of reservation.Instances ?? []) {
            ...
            const item = this.instanceView(account, region, instance, staticIps)
  ...
  async createInstance(account: AwsAccount, region: string, data: Record<string, unknown>) {
    return awsCall('ec2.create_instance', async () => {
      ...
      if (data.root_password)
        input.UserData = Buffer.from(rootPasswordUserData(String(data.root_password))).toString('base64')
      if (data.enable_ipv6) {
  ...
  async startInstance(account: AwsAccount, region: string, id: string) {
    return awsCall('ec2.start_instance', async () => {
      await withAwsRetry('start EC2 instance', () =>
        this.clients.ec2(account, region).send(new StartInstancesCommand({ InstanceIds: [id] }))
      )
    })
  }
  async stopInstance(account: AwsAccount, region: string, id: string) {
    return awsCall('ec2.stop_instance', async () => {
      await withAwsRetry('stop EC2 instance', () =>
        this.clients.ec2(account, region).send(new StopInstancesCommand({ InstanceIds: [id] }))
      )
    })
  }
  async rebootInstance(account: AwsAccount, region: string, id: string) {
    return awsCall('ec2.reboot_instance', async () => {
      await withAwsRetry('reboot EC2 instance', () =>
        this.clients.ec2(account, region).send(new RebootInstancesCommand({ InstanceIds: [id] }))
      )
    })
  }
  async terminateInstance(account: AwsAccount, region: string, id: string) {
    return awsCall('ec2.terminate_instance', async () => {
      ...
      try {
        ...
        throw Object.assign(
          new Error(
            `Elastic IP cleanup failed; instance was not terminated: ${error instanceof Error ? error.message : String(error)}`
          ),
          { cause: error }
        )
      }
      await withAwsRetry('terminate EC2 instance', () =>
  ...
  async allocateStaticIp(account: AwsAccount, region: string, id: string) {
    return awsCall('ec2.allocate_static_ip', async () => {
      ...
      const allocation = await withAwsRetry('allocate EC2 Elastic IP', () =>
        client.send(new AllocateAddressCommand({ Domain: 'vpc' }))
      )
      const allocationId = providerString(allocation?.AllocationId)
  ...
  private instanceView(
    ...
    return {
      ...
      state: providerString(instance.State?.Name),
      public_ip: providerString(instance.PublicIpAddress),
      static_ip: staticIps[id] ?? '',
      ipv6: providerString(instance.Ipv6Address ?? instance.NetworkInterfaces?.[0]?.Ipv6Addresses?.[0]?.Ipv6Address),
      zone: providerString(instance.Placement?.AvailabilityZone),
      instance_type: providerString(instance.InstanceType),
    }
  }
  ...
  private async safeReleaseAddress(client: EC2Client, allocationId: string) {
    await withAwsRetry(
      ...
      () => client.send(new ReleaseAddressCommand({ AllocationId: allocationId })),
  ...
  private async resolveAmi(client: EC2Client, key: string) {
    ...
    const images = [...(result.Images ?? [])].sort((a, b) =>
      providerString(b.CreationDate).localeCompare(providerString(a.CreationDate))
    )
    const ami = providerString(images[0]?.ImageId)
  ...
  private async defaultVpcId(client: EC2Client) {
    ...
    const vpcId = providerString(result.Vpcs?.[0]?.VpcId)
  ...
  private async selectDefaultLaunchSubnet(client: EC2Client) {
    ...
    return providerString(subnets[0].SubnetId)
  }
  ...
  private async selectIpv6Subnet(client: EC2Client) {
    ...
    if (withIpv6.length) {
      ...
      return providerString(withIpv6[0].SubnetId)
    }
    // ensure vpc ipv6
    ...
    const subnetId = providerString(target.SubnetId)
  ...
  private async ensureSubnetIpv6Route(client: EC2Client, subnetId: string) {
    ...
    const routeTableId = providerString(tables?.[0]?.RouteTableId)
    ...
    await withAwsRetry(
      ...
      () =>
        client.send(
          new CreateRouteCommand({ RouteTableId: routeTableId, DestinationIpv6CidrBlock: '::/0', GatewayId: igw })
        ),
