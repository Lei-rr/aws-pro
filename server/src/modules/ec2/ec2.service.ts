...
import { Ec2Provider } from './ec2.client.js'
...
export class Ec2Service {
  ...
  async sync(accountId: string, region: string) {
    ...
    for (const item of await this.instances.all())
      remarks.set(`${item.account_id}|${item.region}|${item.id}`, item.remark ?? '')
  ...
  private normalizeAction(data: Record<string, unknown>) {
    ...
    if (scalarString(data.confirm) !== action)
      throw new ApiError('ec2_action_confirm_required', 'Action confirmation is required', 422, { action })
    return action
  }
