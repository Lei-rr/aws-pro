...
import { awsAccountTags, invalidateAwsCache } from '../../platform/cache/aws-cache.js'
import { maskSecret } from '../../shared/lib/secret-masker.js'
import * as v from '../../shared/aws/aws-validator.js'
import { scalarString } from '../../shared/lib/scalar.js'
import type { AwsAccount, PublicAwsAccount } from '../../shared/aws/aws.types.js'
...
export class AccountService {
  ...
  async update(id: string, body: Record<string, unknown>): Promise<PublicAwsAccount> {
    id = v.accountId(id)
    return withAccountMutation(id, () => this.serialMutation(() => this.updateAccount(id, body)))
  }
  ...
  async delete(id: string): Promise<void> {
    id = v.accountId(id)
    await withAccountMutation(id, () => this.serialMutation(() => this.deleteRecord(id)))
  }

  async runSerialMutation<T>(operation: () => Promise<T>): Promise<T> {
    return this.serialMutation(operation)
  }
