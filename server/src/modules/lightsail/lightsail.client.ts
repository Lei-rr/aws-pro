...
import type { AwsAccount, LightsailInstance } from '../../shared/aws/aws.types.js'
...
export class LightsailProvider {
  ...
  async availabilityZones(account: AwsAccount, region: string): Promise<string[]> {
    return awsCall('lightsail.availability_zones', async () => {
      const result = await this.clients
        .lightsail(account, region)
        .send(new GetRegionsCommand({ includeAvailabilityZones: true }))
      const items: string[] = []
  ...
  async allocateStaticIp(account: AwsAccount, region: string, instanceName: string) {
    return awsCall('lightsail.allocate_static_ip', async () => {
      ...
      await withAwsRetry('allocate Lightsail static IP', () =>
        client.send(new AllocateStaticIpCommand({ staticIpName }))
      )
      try {
  ...
  async deleteInstance(account: AwsAccount, region: string, instanceName: string) {
    return awsCall('lightsail.delete_instance', async () => {
      try {
        ...
        throw Object.assign(
          new Error(
            `Static IP cleanup failed; instance was not deleted: ${error instanceof Error ? error.message : String(error)}`
          ),
          { cause: error }
        )
      }
  ...
  private async ensurePublicIpv4(client: any, instanceName: string) {
    ...
    if (!publicIp) {
      throw new ApiError(
        ...
        'IPv6-only Lightsail instances cannot bind a static IPv4 address',
        422,
        {
