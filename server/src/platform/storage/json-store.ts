...
export class JsonStore<T extends object = Record<string, unknown>> {
  ...
  private async readSnapshot(): Promise<{ value: T; signature: string }> {
    ...
    try {
      ...
      const value =
        content.trim() === '' ? this.clone(this.defaultValue) : ((JSON.parse(content) as T) ?? this.defaultValue)
  ...
  async read(): Promise<T> {
    ...
    if (snapshot.signature === 'missing') {
      await this.createDefaultIfMissing()
      return this.read()
    }
  ...
  private async createDefaultIfMissing(): Promise<void> {
    const filePath = this.absolutePath()
    await this.ensureDirectory()
    let handle: Awaited<ReturnType<typeof fs.open>> | undefined
    try {
      handle = await fs.open(filePath, 'wx', 0o600)
      await handle.writeFile(JSON.stringify(this.defaultValue, null, 2) + '\n', 'utf8')
      await handle.sync()
    } catch (error) {
  ...
  private async acquireLock(lockFile: string): Promise<void> {
    ...
    while (true) {
      try {
        await fs.writeFile(lockFile, JSON.stringify({ pid: process.pid, created_at: Date.now() }), {
          flag: 'wx',
          mode: 0o600,
        })
        return
