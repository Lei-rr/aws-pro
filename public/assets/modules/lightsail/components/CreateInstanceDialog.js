            ...
            <a-spin :spinning="loading">
                <a-form layout="vertical" @submit.prevent="$emit('submit')">
                    <a-row :gutter="16">
                        ...
                        <a-col :xs="24" :md="12"><a-form-item label="IP 类型" required><a-select v-model:value="form.ip_address_type"><a-select-option value="dualstack">双栈（IPv4 + IPv6）</a-select-option><a-select-option value="ipv4">仅 IPv4</a-select-option><a-select-option value="ipv6">仅 IPv6</a-select-option></a-select></a-form-item></a-col>
                        ...
                        <a-col :xs="24"><a-form-item label="套餐" required><a-select v-model:value="form.bundle" placeholder="选择套餐"><a-select-option v-for="(name, id) in options.bundles" :key="id" :value="id">{{ name }}</a-select-option></a-select></a-form-item></a-col>
                    </a-row>
